...
Weekly capacity meters include six daily ticks labelled in the local timezone, counted backwards from the reported reset at 24-hour intervals. The remaining-capacity scale runs from reset (0%, left) to the start of the period (100%, right). A taller foreground tick marks the current linear budget: time remaining divided by seven days. The caption compares remaining capacity with that budget in percentage points, with a one-point neutral band to avoid noisy rounded differences. This is a uniform weekly spending guide, not a forecast of recent request activity. Other window durations, missing reset dates, expired periods, and dates more than seven days ahead do not display this guide.
